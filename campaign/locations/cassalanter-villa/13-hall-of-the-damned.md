...
The cells are empty. They are also functional. The key ring hanging beside them suggests regular use.

**Inhabitants.** None, during non-gathering periods. During cult gatherings (see the cult schedule in the area overview): six crimson-robed acolytes stand at the cell doors, arms crossed, during rites. They are non-combatants — they retreat to A3 if threatened. If the party arrives during a gathering (Caravance midnight or the recurring tenday schedule), the transition from C29 to A1 requires a DC 14 Group Dexterity (Stealth) check; success means the party has twenty seconds before the first acolyte turns toward the staircase. Failure means immediate discovery — two acolytes call out and the gathering in A7 is alerted in the following round.

**Orientation.** Parties entering from A9 (the stream exit) arrive at the far end of the hall — they see the spiral stair ahead, the iron doors to their right. The staircase to C29 is behind them. The direction of progress through the temple is the same regardless of entry point: A1 → A3 → A4/A4a (branching) → A7; A9 is a separate exit path at the temple's lowest level, reachable through lateral passages from A7.
