# Chapter 1: Beginning the Campaign

> **RETIRED** — superseded by modular files in `campaign/guides/players-guide/` and `campaign/guides/gm-guide/`.
>
> Players' Guide: [About This Campaign](../guides/players-guide/about-this-campaign.md) · [Character Creation](../guides/players-guide/character-creation.md) · [Debts of the City](../guides/players-guide/debts-of-the-city.md) · [Bonds and Flaws](../guides/players-guide/bonds-and-flaws.md) · [Faction Affiliations](../guides/players-guide/faction-affiliations.md)
>
> GM Guide: [About This Campaign](../guides/gm-guide/about-this-campaign.md) · [Session Zero](../guides/gm-guide/session-zero.md) · [Debts of the City](../guides/gm-guide/debts-of-the-city.md)

---

## Chapter Purpose

The DM-facing onboarding chapter. Read before session zero. Not referenced during play. Sets expectations, prepares the DM for the session zero conversation, and gives players the information they need to build characters that belong in this campaign.

---

## 1. Core Concept

An urban heist thriller built on community, consequence, and factional intrigue — and the session zero conversation that makes players ready to invest in all three.

---

## 2. Sections

### About This Campaign

**Purpose:** Orients the DM to what kind of game this is before anything else is said to players.

**Content:**
- The three remix pillars (Make It a Heist / Feature All Villains / Fix-Up) stated plainly, one paragraph each
- What distinguishes this campaign from standard D&D: rewards planning over power, all four villain factions simultaneously active, no single right answer about what to do with the gold
- Who this campaign suits (groups that enjoy city exploration, lateral investigation, community stakes, emergent faction storytelling) and who it doesn't (groups that want a primary combat focus or a guided throughline)

**Tone:** Direct, honest. The DM needs to know if this campaign matches their group *before* they start it.

---

### Session Zero Guide

**Purpose:** Gives the DM a script for the pre-campaign conversation. Should feel like advice from someone who has run this before.

**Content:**

*The Conversation Before Character Creation*
- Script for the campaign pitch: "This is a campaign about crime and consequence in a city where everyone has an agenda. Your characters will get tangled up in something much bigger than a job."
- Two things to establish before character sheets come out:
  1. Every useful ally in this campaign has done something questionable. Characters whose ethics prevent all morally grey employment will find large stretches of this campaign frustrating. Be honest about that at the table.
  2. This campaign does not have clean resolutions. Political, moral, and personal situations will end in compromises, tradeoffs, or open wounds. This is intentional.
- DM private note (do NOT share with players): This campaign contains deaths of bonded NPCs, infernal bargains with irreversible consequences, and a villain whose motivations are genuinely sympathetic. None of these should be spoiled at session zero — they land precisely because players do not see them coming.

*The Gold Question*
- Pose this to the table during character creation: **"What would your character do with half a million Dragons?"**
- No right answer. The point is to surface player values before they matter in play. If players disagree vigorously, even better.
- The DM should note what each character wants from the money — this will define how each player experiences the campaign's final choice.

*The Campaign Contract*
- PvP: default is off. If a player wants to join a villain faction, this is a table conversation, not a solo decision.
- The two Zhentarims: establish that in Waterdeep, "the Zhentarim" refers to two unrelated organizations. The Doom Raiders (Davil Starsong's cell) are criminals who present as legitimate businesspeople; they are a joinable player faction. Manshoon's splinter cell is an enemy faction. Characters who join the Doom Raiders are NOT working for the campaign's main antagonist. This distinction causes confusion if not established at session zero.
- Pacing: the campaign rewards players who take initiative. The DM will present a living world, not a guided tour.

*Safety Tools*
- Content categories to disclose (phrased to not spoil):
  - Infernal bargaining and soul forfeiture
  - Child death — a named child dies in the opening event of Fireball; no player intervention is possible
  - Child endangerment with a time-limited resolution window
  - Sudden, unforeseeable loss — characters may lose people they care about
  - Body horror involving transformation
  - Organized crime: extortion, coercion, blackmail, and targeted violence as standard villain tools
- Recommended tools: X-Card for mid-session intervention (especially once the campaign reaches Act III); Lines and Veils at session zero for hard stops and fade-to-black preferences
- DM guidance: if a content moment lands badly mid-session, slow down and check in. The campaign's moral weight is a feature, not a mandate.

---

### Character Creation

**Purpose:** Practical guidance for building characters that belong in Waterdeep and will care about what happens there.

**Content:**

*General Guidance*
- Three things every character needs: a reason to be in Waterdeep, a reason to stay, a reason to involve themselves in strangers' problems
- The inciting incident (finding Floon Blagmaar) works for characters who are mercenary, helpful, or simply in the wrong tavern at the wrong time
- By level 2, the characters own a tavern. Encourage players to want something in Waterdeep beyond the adventure — a reputation, a community, a debt, a dream

*Species Notes (2024)*
- Drow: will encounter Jarlaxle and Bregan D'aerthe; Jarlaxle has complicated feelings about surface drow; coordinate with DM before session one
- Gnome: natural connection to House of Inspired Hands (Arc C); can build a relationship with Nim the automaton-maker
- Tiefling: the Cassalanters regard tieflings as distant infernal kin and will show warmth that deserves scrutiny

*Backgrounds (XPHB — 2024 Player's Handbook)*

All 16 XPHB backgrounds, rated for Waterdeep fit:

| Background | Fit | Waterdeep Hook |
|-----------|-----|----------------|
| Criminal | Excellent | Former guild operative; still owes favors to someone still active |
| Guard | Excellent | Ex-City Watch; knows which laws bend and which captains take coin |
| Artisan | Strong | Guild system is everything in Waterdeep; knows the permit labyrinth firsthand |
| Charlatan | Strong | Social deception is currency here; cover identities feel natural |
| Entertainer | Strong | Sea Maidens Faire connections; experience performing under pressure |
| Merchant | Strong | Trade district roots; understands which guild holds which territory |
| Noble | Strong | Sea Ward social access; invitations to events that matter |
| Sailor | Strong | Dock Ward roots; knows what the Sea Maidens Faire actually carries |
| Acolyte | Good | Temple ties — Tyr and Tymora are visible; Asmodeus has a hidden temple |
| Sage | Good | Knowledge is commodity here; House of Inspired Hands connections |
| Scribe | Good | Information-network connections; Zhentarim employs scribes |
| Soldier | Good | Military background translates to City Watch and mercenary contexts |
| Wayfarer | Good | Knows roads, inns, and safe houses; useful in a city of strangers |
| Guide | Situational | Knows the city's layout in ways that matter during a heist |
| Hermit | Situational | Unusual in a city; strong contrast character if the seclusion has a story |
| Farmer | Situational | Recent transplant; genuine outsider perspective on the city's strangeness |

*Backgrounds (FRHoF — Forgotten Realms: Heroes of Faerûn)*

Select FRHoF backgrounds suited to a Waterdeep campaign:

| Background | Fit | Waterdeep Hook |
|-----------|-----|----------------|
| Harper | Excellent | Mirt the Moneylender is a Harper; this background creates an immediate named contact |
| Lords' Alliance Vassal | Excellent | Waterdeep leads the Lords' Alliance; political access and faction credibility from day one |
| Zhentarim Mercenary | Excellent | Direct tie to the Doom Raiders; the Two Zhentarims distinction makes this character's loyalty a live question |
| Shadowmasters Exile | Strong | Criminal underworld connections; the Shadowmasters have reach into Waterdeep's black market |
| Chondathan Freebooter | Strong | Sword Coast maritime culture; natural in a major port city |
| Mythalkeeper | Good | Arcane history of Waterdeep runs deep; connections to elven lore and high magic |
| Emerald Enclave Caretaker | Good | Waterdeep's parks and green spaces are Enclave-protected; Melannor Fellbranch is an immediate contact |
| Knight of the Gauntlet | Good | Order of the Gauntlet; Hlam on Mount Waterdeep is their contact in the city |
| Spellfire Initiate | Good | Unusual magical origin; draws attention from factions interested in rare arcane assets |
| Flaming Fist Mercenary | Situational | From Baldur's Gate; recent transplant with a mercenary's outlook |
| Genie Touched | Situational | Calimshan origin; distinctive outsider with magic suited to social infiltration |
| Mulhorandi Tomb Raider | Situational | Scholar angle; investigation-focused character with unusual knowledge base |
| Dragon Cultist | Situational | Requires table conversation; interesting for a character with a hidden allegiance |
| Ice Fisher | Situational | From Icewind Dale; character with survival skills far from home |
| Moonwell Pilgrim | Situational | Moonshae Isles origin; nature-focused with an otherworldly perspective on the city |
| Purple Dragon Squire | Situational | Cormyr-specific origin; limited natural connection to Waterdeep's power structure |
| Rashemi Wanderer | Situational | From Rashemen; strong character concept but minimal natural faction ties |
| Dead Magic Dweller | Situational | Anauroch background; interesting for a character with a deliberate relationship to magic |

---

*Debts of the City*

**Purpose:** Waterdeep runs on favors, markers, and binding agreements. The city's power brokers — noble houses, criminal guilds, moneylenders, and worse — rarely deal in simple coin. They deal in leverage.

Each player chooses one Debt of the City: a specific, lingering obligation that exists before the campaign begins. Unlike a backstory event, a Debt doesn't end — it follows the character. A social and narrative anchor, it plants them inside the city's power structures and the Grand Game before the first session ends.

> **Design Note: Debts as Acceleration.** The Debt system exists to solve a classic sandbox problem: players who feel like observers of a world that hasn't yet involved them. A character who owes Istrid Horn a debt is already inside the story. A character who ran Floxin's package has a problem that will find them whether or not they go looking. Space the debt's demands across the campaign — early calls should be low-stakes and character-defining; mid-campaign calls should create genuine tension; late-campaign calls should force meaningful choices about loyalty, identity, and the price of freedom. Use debts as reasons for NPCs to seek the characters out, not as punishments.

Roll or choose from the table below, then read the full entry.

| d8 | Motivation | Debt |
|----|-----------|------|
| 1 | Ambition | Istrid Horn's Ledger |
| 2 | Survival | Floxin's Package |
| 3 | Entanglement | The Gralhund Seal |
| 4 | Purpose | Renaer's Confidence |
| 5 | Redemption | Ahmaergo's Inventory |
| 6 | Loyalty | Davil's Asset |
| 7 | Justice | Blastwind's Evidence |
| 8 | Knowledge | Skeemo's Index |

---

**Ambition — Istrid Horn's Ledger**

You needed coin for something — a venture, a passage, a crisis — and Istrid Horn of the Doom Raiders provided it. The terms were fair, the interest reasonable, and the half-dwarf woman behind the table was businesslike to the point of tedium. What made you uneasy afterward was how little she seemed to need reassurance that you would pay.

The loan has since been partially repaid. What remains is not enough to panic over. What remains is enough to keep you in Istrid Horn's ledger, and her ledger is — as best you can determine — flawless. Every entry. Every outstanding balance. Every note about the debtor's known associates.

*Your character knows the following:* Istrid Horn holds a legitimate debt. She has made no contact outside of routine payment collection. You are not afraid of her. You are merely aware that she knows a great deal about you, has recorded it, and will continue to add to the record.

**The Holder: Istrid Horn (Doom Raiders)**
A half-dwarf who runs a money-lending operation out of the South Ward and serves as one of Davil Starsong's most effective lieutenants. She doesn't deal in charm. Reliable, exact, and genuinely disinterested in personal drama — she wants debts paid and useful people identified. Her first contact with the character arrives through routine channels, a note about a payment, a clarifying question about an arrangement. It is only later that it becomes clear she has been forwarding summaries of the debt relationship to Davil Starsong. Who the character owes, and how reliably they pay, tells her everything about who they are.

---

**Survival — Floxin's Package**

You were hired — through enough layers of intermediary that you could not have known at the time — to carry a sealed package from the Trades Ward to a contact in the Dock Ward and ask no questions. You did not ask questions. You carried the package. When you arrived at the delivery point there was a body, fresh, and three armed people who did not work for whoever had hired you. You ran. The package came with you.

The package is still with you, still sealed. You haven't opened it. The sensible knowledge that some things can't be unlearned is its own kind of discipline. The package is heavier than it looks, and sometimes warmer.

*Your character knows the following:* Urstul Floxin — a name you obtained with difficulty afterward — is the man who was waiting for the package. He has made one attempt to locate you, through channels that required real effort to trace back. He knows approximately where you are. He has not moved against you, which means either he is biding his time or he wants the package more than he wants satisfaction.

**The Holder: Urstul Floxin (Manshoon's Zhentarim)**
One of Manshoon's most effective field lieutenants: methodical, personally dangerous, and not given to visible anger. His first approach to the character will be dressed as a business transaction — a reasonable offer for the package's return, no questions asked, the whole matter tidied away. He means this, up to a point. The offer becomes less reasonable each time they decline. Characters who return the package and survive the meeting will discover they've demonstrated exactly the competence Floxin prefers in reluctant assets. The relationship doesn't end with the exchange — it reorganizes around a new one.

> **Warning: Two Zhentarims.** Floxin works for Manshoon, not the Doom Raiders. A character whose debt is held by Floxin is already in the cross-hairs of the campaign's villain faction, whether they know it or not. Davil Starsong's cell will not automatically help this character — they are a separate organization. Let the character navigate the distinction without the DM explaining it for them.

---

**Entanglement — The Gralhund Seal**

Noble house seals are not difficult to obtain if one is patient and moves in the right circles. The Gralhund family crest — sword above shield, in green and silver — opened a door that the character's actual credentials never would have. A letter of introduction signed with that seal got them into a building, a meeting, an arrangement. They didn't forge it themselves — they paid someone to. The distinction matters to them and to no one else.

What they did not know when they acquired the credential was that Yalah Gralhund is meticulous about her family's name, employs people whose entire purpose is the protection of that name, and has recently become extremely attentive to unauthorized use of her correspondence.

*Your character knows the following:* The seal was used. The credential exists somewhere in someone's records. Whether it has been traced is not known. What is known is that Yalah Gralhund is not the kind of person who discovers an insult and simply absorbs it.

**The Holder: Yalah Gralhund**
A Waterdhavian noble of genuine intelligence and practical ruthlessness — her husband Orond manages appearances; she manages the house. Her first contact with the character will feel like an invitation: social, professional, warm without being warm. She won't accuse. She'll place the character in a position where they understand, without being told, that she knows — and that her response isn't yet decided. Characters who handle this meeting well, demonstrating they are worth more as an asset than as an example, may find the credential problem quietly retired in exchange for a specific private service. Characters who confirm her suspicion that they are merely opportunistic will have a different experience.

---

**Purpose — Renaer's Confidence**

Renaer Neverember trusted you with something he does not trust easily: the truth. Not the public truth about his father's corruption — that story is becoming common knowledge — but a specific, private piece of it: a name, a location, a transaction that implicates someone who is not his father — still active, still in Waterdeep. He told you because he needed someone outside his usual circles to hold the information. Because he thought you would know what to do with it if something happened to him.

Nothing has happened to him. Yet. But the information is still yours, and what you do with it — keep it, use it, trade it — will define what kind of person you are in a city that runs on exactly this kind of knowledge.

*Your character knows the following:* You are carrying information that multiple powerful people would very much want. Renaer does not yet know how much he can trust you with it. The information's value will not last forever.

**The Holder: Renaer Neverember**
The son of Waterdeep's disgraced former Open Lord: educated, wealthy, politically exiled by association with his father's crimes, and working quietly to repair both his city and his name. He's genuinely decent, which makes him dangerous in a different way than the people around him — decent people are harder to predict when pushed far enough. He won't demand the information back. He won't threaten. He'll ask, once, whether the character has done anything with it, in a tone that makes clear he has thought carefully about what the possible answers mean. Characters who have acted honorably gain a friend who's more connected than he appears. Characters who haven't will find that Renaer's decency has a limit they've helped define.

---

**Redemption — Ahmaergo's Inventory**

The job was supposed to be clean. The Xanathar Guild was acquiring something from a rival organization; the character was hired to facilitate transit and paid well enough not to ask questions. What came out of there wasn't what they'd been told. What they left behind was considerably worse. The Guild got its item. The irregularities were absorbed into a larger accounting of that night, and the character's involvement was not officially noted.

Ahmaergo — Xanathar's dwarven majordomo, the man who actually runs the organization's logistics — keeps a private ledger separate from the official records. The character's name is in it, under a column that reads *Outstanding.*

*Your character knows the following:* The Guild does not know, officially, that you exist. Ahmaergo knows. He made one indirect inquiry months after the event, to confirm you were still in Waterdeep and still capable. You confirmed both by answering.

**The Holder: Ahmaergo (Xanathar's Guild)**
A compact, humorless dwarf who is in practical terms the operational brain of the Xanathar Guild. He doesn't have Xanathar's moods or its capacity for sudden violence, which makes him considerably more dangerous to deal with: he's consistent. His first formal request arrives as a logistics problem — something that requires someone unaffiliated, deniable, and competent. He won't call it debt repayment. Characters who complete the task will find the *Outstanding* column now reads *Current.* Characters who refuse will find the official records suddenly contain their name in a context that requires immediate explanation to the City Watch.

> **Warning: The Beholder.** Ahmaergo maintains his private ledger precisely because Xanathar does not know about every arrangement. If Xanathar becomes aware of the character's existence — through carelessness, through another character's indiscretion, through any direct contact — the dynamic changes completely. Xanathar does not distinguish between asset, debtor, and enemy when its paranoia is engaged. Keep the beholder out of this relationship for as long as dramatically possible.

---

**Loyalty — Davil's Asset**

You did not know, for some time, that you were being run. The information you received — dropped at predictable points, offered through trustworthy intermediaries, always accurate — was too useful to refuse. You used it. It improved your situation in Waterdeep in specific, concrete ways. Only later, reviewing what you had been given and when, did you realize that someone had been carefully managing what you knew and what you did with it. That someone, it eventually became clear, was Davil Starsong.

Davil came afterward to offer a drink and mention, warmly, that he'd heard good things about you and thought it was past time you met properly. No acknowledgment of the arrangement — just an introduction.

*Your character knows the following:* Davil Starsong used you as an intelligence asset without your consent, and his first official meeting with you opened with a compliment rather than an admission. This tells you most of what you need to know about how he operates — not what he intends next.

**The Holder: Davil Starsong (Doom Raiders)**
A slender, elegantly dressed sun elf who serves as the public face of the Doom Raiders' Waterdeep operation. He is charming, cultured, and entirely transparent about being charming and cultured — which is its own form of deflection. Beneath the social performance, he's a genuine professional who believes the Doom Raiders' moderate approach to crime is both more profitable and more ethical than Manshoon's methods. His first formal ask of the character is small and explicitly acknowledged as such — he's calibrating them, not exploiting them, and he makes this clear. Characters who engage honestly will find the relationship evolving into something closer to a genuine working partnership. Characters who try to leverage the asset-running against him will discover that Davil has been planning for that move since the first note was dropped.

---

**Justice — Blastwind's Evidence**

Barnibus Blastwind is a City Watch investigator of considerable reputation: thorough, methodical, rarely wrong. You watched Blastwind do something deliberate. When he realized he'd been seen, he made the best decision available to a compromised man: he did nothing. Held your gaze for a long moment, then went back to what he'd been doing.

A week later, an unsigned note arrived at your residence. It said: *We should meet. You have seen something. I have a file.* When the meeting happened, the file contained everything Blastwind had been able to gather on you in seven days. He wasn't there to threaten you. Call it an introduction.

*Your character knows the following:* Barnibus Blastwind knows what you saw. You know what he did. Both of you are now holding something the other cannot comfortably use without mutual destruction. Blastwind seems to find this arrangement workable.

**The Holder: Barnibus Blastwind (City Watch)**
An investigator in his middle years — reputation earned, private compromise carefully buried. His compromise is situational: one incident, singular, involving a person he refuses to discuss. He's also genuinely good at his job, which makes him valuable in ways the character didn't anticipate when this began. His first official request involves something the Watch can't pursue through official channels. He frames this honestly. Characters who help him begin accumulating something more durable than leverage: a genuine ally who knows the city's investigative apparatus from the inside, and who has a reason to keep them safe.

---

**Knowledge — Skeemo's Index**

Skeemo Weirdbottle maintains, out of a cramped and thoroughly unassuming shop in the Dock Ward, one of the most comprehensive indices of Waterdeep's criminal and semi-criminal activities that exists outside the Xanathar Guild's own records. He compiled it partly for business purposes and partly because he finds the city's power structures intrinsically interesting as a subject of study. He sold you something from it — access, a specific piece of information, a document that enabled something you needed to do — and conducted the transaction with professional discretion.

What he did not mention was that he had already entered it into the index as he filed it. Under your name. With a full record of what you purchased, what it suggests you were trying to accomplish, and a brief speculative notation about your likely future needs.

*Your character knows the following:* Skeemo Weirdbottle has detailed knowledge of at least one thing you have done, filed under your name, cross-referenced against other entries. He doesn't threaten you with it. He appears to simply consider it useful to know. Whenever you encounter him, he'll offer to sell you additional information, and the prices are always fair. This, somehow, makes it worse.

**The Holder: Skeemo Weirdbottle (Doom Raiders)**
A gnome alchemist whose cheerful manner and genuine intellectual curiosity make it easy to miss that he's one of the most effectively networked people in Waterdeep's criminal underground. He serves the Doom Raiders primarily as an information broker, a supplier, and an analyst, and he is better at all three than anyone outside the organization knows. His requests, when they come, tend to be strange: he wants something specific and counterintuitive, and he pays well for it. Characters who indulge his particular interests may find that his index begins working in their favor — information about other people surfacing at convenient moments. Characters who try to find and destroy his records will discover that Skeemo maintains backups in more places than one, and was genuinely more amused than alarmed by the attempt.

---

*Bonds and Flaws*

**Bonds** are formative backstory elements — the wound or truth those events left, not the events themselves. A Bond explains why the character behaves the way they do: the thing that shaped their worldview, their relationship to power, their capacity for trust. Every Bond connects to something the campaign will press on.

**Flaws** are the character's internal arc — each one a thing the character must overcome by the end of the campaign to become who they need to be. These are genuine internal obstacles, not personality flavoring or DM tools. Each one answers: *what is this character's damage, and what does it cost them?* The campaign's events — the fireball, the Cassalanter children, the vault, the gold — are designed to press on exactly these.

Players choose or roll one Bond and one Flaw. Ask both to be read aloud at the table during session zero — the conversation that follows is usually more useful than any individual entry.

> **Design Note: Tracking the Arc.** The DM should note each character's Flaw at session zero and track whether it is being tested. A Flaw that never surfaces in play is a missed opportunity. The campaign's natural pressure points are built to engage all eight Flaws — but only if the DM is watching for the moment. The resolution of a Flaw is not a mechanical event. It is a scene: a choice, under pressure, where the character does the thing their Flaw would have prevented. That scene should feel earned.

**Bonds (d8)**

| d8 | Bond |
|----|------|
| 1 | I watched someone with power use the law to destroy someone without it — cleanly, legally, without consequence. I have not been able to look at institutions the same way since. |
| 2 | I was given a second chance by someone who had no reason to give it. I have spent the years since trying to justify their faith, and failing often enough that I still don't trust it was deserved. |
| 3 | I built something once — a business, a community, a relationship — and watched it taken apart by forces I couldn't fight. I am building again. I do not talk about the first time. |
| 4 | I grew up understanding that the powerful protect their own and no one else. That understanding has made me effective at surviving, and nearly incapable of believing anyone wants to help without wanting something back. |
| 5 | I was loyal to someone who turned out not to deserve it. The discovery was catastrophic. I am still learning to distinguish loyalty that is earned from loyalty that is simply comfortable. |
| 6 | Someone cared about me before I had done anything to earn it. That care was real, and it made me capable of caring about others. That person is no longer here. |
| 7 | I have spent long enough pretending to be something I am not that I am genuinely uncertain, on some days, what I actually am underneath the performance. |
| 8 | I came from somewhere that does not prepare you for a city like Waterdeep. The city has changed me in ways that would disturb the people I grew up with. I have not decided whether that is growth or loss. |

**Flaws (d8)**

| d8 | Flaw | Arc it connects to |
|----|------|--------------------|
| 1 | I believe the world owes me a better deal than it has given me. Until I stop keeping that accounting, every choice I make will be shaped by it. | Arc J: what you do with the gold is the final score |
| 2 | I have decided, without admitting it to myself, that keeping myself safe matters more than doing right. | Arc C: the fireball forces the question |
| 3 | I cannot forgive. Not really. I perform forgiveness convincingly when it suits me. | Arc G: the Cassalanters are parents who did something unforgivable for the right reasons |
| 4 | I use competence as a substitute for connection. As long as I am useful, I do not have to be known. | Arc B: the Trollskull community forms around the tavern and will not let this stand |
| 5 | I have decided that some people are not worth protecting. The category quietly expands when I am tired or afraid. | Arc G: are the Cassalanter children worth the cost of saving them? |
| 6 | I need to be right more than I need to be honest. The gap between those two things is larger than I have allowed myself to see. | Faction loyalty: the Doom Raiders will eventually ask for something uncomfortable |
| 7 | I have mistaken control for safety for so long that I cannot tell the difference. I plan. I manage. I do not trust. I call this wisdom. | The heists: this cannot be done alone |
| 8 | I am waiting for someone to give me permission to want what I want. No one is going to give that permission. I know this. I am still waiting. | Arc J: the vault is the moment — and no one is going to tell you what to do |

---

### Faction Affiliations

**Purpose:** Gives players enough information to form preferences before session one, without making a binding choice before they've met the game's faction contacts in play.

**Content:**
- No faction is chosen at session zero. Players express preferences; formal affiliations are established when faction contacts approach the PCs during Chapter 2 (Arc B). The campaign makes this natural — every major faction initiates contact during the Trollskull Alley arc.
- One exception: Bregan D'aerthe cannot be chosen at session zero. Jarlaxle enters the campaign only if the players follow the nimblewright investigation in Arc C, which leads to the Sea Maidens Faire. If that thread is never followed, Bregan D'aerthe remains adversary or neutral. Interested players should know this before getting their heart set on it.
- The Two Zhentarims note must appear here (not just in the contract section above): a player who wants to join "the Zhentarim" needs to understand they are choosing the Doom Raiders, not Manshoon's cell. These are distinct organizations with different contacts, methods, and moral postures.

**Preview Table** (for player reference at session zero, not a selection screen):

| Faction | Contact (met in Arc B) | What they offer | What they ask |
|---------|------------------------|-----------------|----------------|
| Harpers | Mirt the Moneylender | Intelligence, political cover, archive access | Surveillance, dead drops, attending specific events |
| Lords' Alliance | Jalester Silvermane | Official sanction, noble-tier social access, legal protection | Reporting on crime, protecting civic stability |
| Order of the Gauntlet | Hlam (Mount Waterdeep hermit) | Reliable combat backup in righteous causes | Actively exposing evil, especially infernal dealings |
| Emerald Enclave | Melannor Fellbranch | Safe passage through parks, ranger contacts | Protecting natural spaces; opposing undead activity |
| Force Grey | Vajra Safahr | Powerful backup in genuine city-scale threats | Exclusive — must earn Vajra's trust first |
| Doom Raiders (Zhentarim) | Davil Starsong | Criminal network access, fences, safe houses, street intelligence | Jobs that keep the network profitable |
| Bregan D'aerthe | Jarlaxle (as "Zardoz Zord") | Best intelligence in the city; drow resources | Loyalty — and eventually, a political favor for Luskan |

- Bregan D'aerthe conditional entry note: this faction is only available if the players follow the nimblewright investigation to the Sea Maidens Faire in Arc C. Discuss with any interested player now so they understand it is a path to unlock, not a session zero pick.
- Conflicting loyalty note: a party split between the Doom Raiders and Bregan D'aerthe restructures Arc H significantly. This is manageable but requires a table conversation before it happens.

---

## 3. Key Decisions Before Writing

1. **Bregan D'aerthe conditional entry:** The chapter must make unmistakably clear that Bregan D'aerthe is not a session zero faction choice. The conditional entry mechanic (Arc C nimblewright → Sea Maidens Faire → Jarlaxle contact) should be explained with enough specificity that a player understands what they need to do in play to unlock it — without spoiling Arc C's investigation.

2. **Tone of the chapter:** Not sales copy. Not hype. Say what the campaign is and isn't. A group that reads this and decides it isn't their game has been well-served.

3. **The gold question placement:** "What would your character do with half a million Dragons?" should appear in the session zero script as a player-facing prompt, not in the DM's private design notes. It surfaces player values. The answer matters later.

---

## 4. Source References

- `adventure-wdh.json` Introduction section (lines ~424–1579): "Running the Adventure," "Adventure Structure," "Character Creation," "Factions in Waterdeep" — read for original framing to distinguish and improve upon
- `adventure-wdh.json` Ch. 2 lines ~2993–4330: faction contacts introduced during Trollskull Alley
- `3. Player Character Factions.pdf`: faction contacts, renown tiers, what each faction offers — primary reference for faction table content
- `sources/backgrounds.json` (5etools-mirror-3): verified source for XPHB and FRHoF background data
- `adventure-reloaded` skill: NPC profile format (if any faction contact gets an inline profile here)

---

## 5. Cross-References

- Arc A: Volo, Durnan, and Renaer are met in session one; Debts of the City may name them or connect to them
- Arc B: Faction contacts make their recruitment pitches here; no faction is locked until this arc
- Arc C: Bregan D'aerthe conditional entry is gated behind this arc's nimblewright investigation
- Arc J: "What would your character do with half a million Dragons?" from session zero is answered here
- Chapter 3: Fireball victim guidance and candidate NPC list goes in Chapter 3, not here
- Appendix C (Player Factions): the faction table above is a preview; full reference is in Appendix C
- Appendix D (Running Player Factions): renown and mission system referenced but not detailed here
