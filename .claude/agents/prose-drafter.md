...
- Quest and location pages use the three foundry-journal components, not plain blockquotes: every read-aloud box is `> [!narrative]`; the players' first sight of an NPC is `> [!npc-narrative]**Name**` (first meeting only); every player question with an NPC answer is `> [!dialogue]**Question**`. These are not sidebars and don't count toward the brief's sidebar limit. The text inside `[!narrative]` and `[!npc-narrative]` boxes is read-aloud: write it to be spoken to the players, following the Read-Aloud Text section of `dnd-adventure-text` (perceivable only, present tense, interrupted action, 3–5 sentences, ends unresolved). Never GM notes, summaries or backstory.
