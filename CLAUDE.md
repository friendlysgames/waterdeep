...
**Agents do the work; resume them after rate limits:** Conversion, drafting, research, and polishing go to the project agents in `.claude/agents/`. The main session plans, reviews, applies small review fixes, and commits. If an agent stops on a rate limit, the limit is account-wide: after it resets, continue that same agent with SendMessage. Never take its work over inline.
